...
func (s *Storage) DeleteURL(urlToDelete string) error {
	...
	if err != nil {
		return fmt.Errorf("%s: failed to get row affected: %w", op, err)
	}

	if count == 0 {
		return fmt.Errorf("%s: no rows deleted for URL: %s", urlToDelete)
	}
